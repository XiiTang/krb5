...
def run(args,env):
    ...
    return result
with tempfile.TemporaryDirectory(prefix='imapipe-kdc-') as temp:
    p=Path(temp);os.chmod(p,0o700)
    with socket.socket() as s:s.bind(('127.0.0.1',0));port=s.getsockname()[1]
    config=p/'krb5.conf'
    config.write_text(f'''[libdefaults]
 default_realm = IMAPIPE.TEST
...
''')
    ...
    run([BUILD/'kadmin/dbutil/kdb5_util','create','-s','-P','owned-fixture-master-password','-r','IMAPIPE.TEST'],env)
    for principal in ['user','imap/server.test']:
        run([BUILD/'kadmin/cli/kadmin.local','-r','IMAPIPE.TEST','-q',f'addprinc -randkey {principal}'],env)
    for principal,name in [('user','user.keytab'),('imap/server.test','server.keytab')]:
        run([BUILD/'kadmin/cli/kadmin.local','-r','IMAPIPE.TEST','-q',f'ktadd -k {p/name} -norandkey {principal}'],env)
    log=(p/'kdc.log').open('w')
    server=subprocess.Popen([str(BUILD/'kdc/krb5kdc'),'-n','-r','IMAPIPE.TEST'],env=env,stdout=log,stderr=log)
    try:
        ...
        run([BUILD/'clients/kinit/kinit','-k','-t',p/'user.keytab','user@IMAPIPE.TEST'],env)
